...
class BitgetTickerMapper:
    """
    Maps Bitget WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'bitget') -> Optional[int]:
        """
        Get vendor ID for Bitget.

        Args:
            vendor_name: Vendor name (default: 'bitget')
    ...
    def map_bitget_ticker_fields(self):
        """
        Map Bitget WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting Bitget ticker field mapping...")

        # Get vendor and channel IDs
        vendor_id = self.get_vendor_id('bitget')
        if not vendor_id:
            logger.error("Bitget vendor not found")
            return 0

        channel_id = self.get_websocket_channel_id(vendor_id, 'ticker')
        if not channel_id:
            logger.error("Bitget ticker channel not found")
            return 0
        ...
        mappings = [
            ...
            ('data.askSz', 'best_ask_size', {'type': 'string_to_numeric'}, 'ticker'),

            # High/Low/Open fields (string to numeric conversion)
            ('data.high24h', 'high_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ...
            ('data.ts', 'timestamp', {'type': 'integer_to_datetime'}, 'ticker'),
        ]
        ...
        logger.info(f"Bitget ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        # Count total mappings for Bitget ticker
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'bitget' AND data_type_name = 'ticker'
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Map Bitget WebSocket ticker fields to canonical fields",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create Bitget ticker mappings
  python create_bitget_mappings.py

  # Create mappings and verify results
  python create_bitget_mappings.py --verify

  # Dry run (show what would be mapped without actually creating)
  python create_bitget_mappings.py --dry-run
        """
    )
    ...
    with BitgetTickerMapper() as mapper:
        if args.dry_run:
            ...
            print()
            print("Total: 12 potential mappings")

        else:
            # Create mappings
            created = mapper.map_bitget_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} Bitget ticker field mappings")
