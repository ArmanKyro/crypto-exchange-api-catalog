...
class TemplateAdapter(BaseVendorAdapter):
    """
    Template adapter for [EXCHANGE_NAME] Exchange API.

    Replace all occurrences of:
    - 'TemplateAdapter' with '[ExchangeName]Adapter' (e.g., 'BybitAdapter')
    - '[EXCHANGE_NAME]' with actual exchange name (e.g., 'Bybit')
    - '[BASE_URL]' with actual REST API base URL
    - '[WEBSOCKET_URL]' with actual WebSocket URL
    ...
    def discover_rest_endpoints(self) -> List[Dict[str, Any]]:
        """
        Discover [EXCHANGE_NAME] REST API endpoints.
        ...
        """
        logger.info("Discovering [EXCHANGE_NAME] REST endpoints")
    ...
    def discover_websocket_channels(self) -> List[Dict[str, Any]]:
        """
        Discover [EXCHANGE_NAME] WebSocket channels and message formats.
        ...
        """
        logger.info("Discovering [EXCHANGE_NAME] WebSocket channels")
    ...
    def discover_products(self) -> List[Dict[str, Any]]:
        """
        Discover [EXCHANGE_NAME] trading products/symbols from live API.
        ...
        """
        logger.info("Discovering [EXCHANGE_NAME] products from live API")
        ...
        try:
            ...
            if not isinstance(symbols_data, list):
                ...
                raise Exception(f"Unexpected response format from [EXCHANGE_NAME]")
            ...
            raise Exception(f"Product discovery failed for [EXCHANGE_NAME]: {e}")
